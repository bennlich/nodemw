...
(function(exports) {
	...
	bot.prototype = {
		...
		getToken: function(title, action, callback, errback) {
			var promise = this.api.call({
				action: 'query',
				prop: 'info',
				intoken: action,
				titles: title
			...
			promise.then(function(res) {
				...
				if (!token) {
					console.log("Did not receive edit token.");
					errback && errback();
		...
		edit: function(title, content, summary, callback, errback) {
			var self = this;

			// @see http://www.mediawiki.org/wiki/API:Edit
			var promise = this.getToken(title, 'edit');
			promise.then(function(res) {
				var	token = res.token;
		...
		'delete': function(title, reason, callback, errback) {
			var self = this;
		...
		getUserContribs: function(options, callback, errback){
			options = options || {};
			...
			this.api.fetchUrl(url, 'binary').then(function(content) {
                ...
                getExternalLinks: function (title, callback) {
                        this.api.call({
                                ...
                                callback((data && getFirstItem(data.pages).extlinks) || []);
                        });
                },
