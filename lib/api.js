...
(function(exports) {
	...
	// get an item out of an object
	var getAnItem = function(object) {
	...
	var doRequest = function(params, callback, method, promise) {
		...
		var req = this.http.request({
			...
			res.on('end', function() {
				...
				if (info) {
					// don't resolve the promise if you expected a token and got none
					if (params.intoken) {
						var token = getAnItem(info.pages)[ params.intoken + 'token' ];
						if (token) {
							promise.resolve(promiseRes);
						}
						else {
							promise.reject(promiseRes);
						}
					}
					else {
						promise.resolve(promiseRes);
	...
	var addToQueue = function(fn, callback, errback) {
		var promise = new Deferred();

		// when the promise resolves/rejects, call the callback/errback
		promise.then(
			function(res) {
				callback && callback(res.info, res.next, res.data);
				return res;
			},
			function(res) {
				errback && errback(res.value.data.error);
				return res;
			}
		);
	...
	api.prototype = {
		...
		call: function(params, callback, errback, method) {
			var self = this;
